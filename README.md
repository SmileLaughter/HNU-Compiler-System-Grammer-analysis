...
此外，由于拔尖班的编译课程可能和其他专业在细节处的格式上**略有差异**，请同学们自行甄别，但整体逻辑肯定是一样的。如有需要可以再让 AI 进行修改，或者在运行程序后人工修改。
...
项目使用文件输入形式，支持“文法 + 句子”格式：

- 将要测试的文法与句子放在 input 文件夹下，任意一个 `.txt` 文件中，使用 `%%` 分隔符将文法部分和句子部分隔开，示例如下：
- 如果不需要解析句子，则可以省略 `%%` 分隔符和句子。
...
- 句子部分：每行一个句子
  - 句子可以使用空格作为分隔符，例如：`id + id * id`，程序会按空格分割为符号列表
